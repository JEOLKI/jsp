...
public class RedirectController {
	...
	public String process(Model model, HttpSession session, RedirectAttributes ra) {
		...
		ra.addAttribute("msg_ra_attr", "brown"); // 모델객체를 못읽기때문에 ra의 속성에추가한다.
		
		// 리다이렉트시 모델 객체에 추가된 속성을 리다이렉트 주소의 파라미터로 추가한다(get) 리다이렉트 어트리뷰트 안썻을때
		// "redirect:/login/view" ==> /login/view?msg=hello, world
		
		return "redirect:/login/view";  // http://localhost/login/view?msg=hello%2C+world 직접써도된다.
	}
