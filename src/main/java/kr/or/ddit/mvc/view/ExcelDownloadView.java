...
public class ExcelDownloadView extends AbstractView{
	...
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request,
		...
		// 첨부파일임을 암시 
		response.setHeader("Content-Disposition", "attachment; filename=test.xlsx"); // filename은 controller에서 넣어줘서 사용가능
		...
		int rownum = 0;
		int column = 0;
		Row row = sheet.createRow(rownum++); // 행에대한 관리를 해야한다.
		...
		for(Map<String, String> map : data) {
			row = sheet.createRow(rownum++); // 행에대한 관리를 해야한다.
